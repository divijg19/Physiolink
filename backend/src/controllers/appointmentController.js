...
exports.createAvailability = async (req, res) => {
	try {
		const result = await appointmentService.createAvailability(
			req.user.id,
			req.body.slots || [],
		);
...
exports.getTherapistAvailability = async (req, res) => {
	try {
		const availableSlots = await appointmentService.getTherapistAvailability(
			req.params.ptId,
		);
...
exports.bookAppointment = async (req, res) => {
	try {
		const updated = await appointmentService.bookAppointment(
			req.params.id,
			req.user.id,
		);
