...
async function getAllTherapists(params = {}) {
	...
	const filters = [];
	if (specialty)
		filters.push({ "profile.specialty": { $regex: specialty, $options: "i" } });
	if (location)
		filters.push({ "profile.location": { $regex: location, $options: "i" } });
			...
			pipeline: [
				...
				...(dateFilter
					? [
							...
							{ $match: { startDateString: dateFilter } },
						]
					: []),
				{ $sort: { startTime: 1 } },
				{ $limit: 20 },
	...
	basePipeline.push({
		$addFields: { availableSlotsCount: { $size: "$availableSlots" } },
	});
	basePipeline.push({
		$lookup: {
			from: "reviews",
	...
	basePipeline.push({
		$addFields: {
			reviewCount: { $ifNull: [{ $arrayElemAt: ["$reviewAgg.count", 0] }, 0] },
		},
	});
	basePipeline.push({ $project: { password: 0, reviewAgg: 0 } });

	if (requireAvailable)
		basePipeline.push({ $match: { availableSlotsCount: { $gt: 0 } } });

	const pipeline = [
		...basePipeline,
		{
			$facet: {
				results: [{ $sort: sortStage }, { $skip: skip }, { $limit: limitNum }],
...
async function getTherapistById(therapistId, dateFilter) {
	const pipeline = [
		...
		},
		{
			$addFields: {
				reviewCount: {
					$ifNull: [{ $arrayElemAt: ["$reviewAgg.count", 0] }, 0],
				},
			},
		},
