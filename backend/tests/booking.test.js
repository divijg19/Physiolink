...
test('concurrent booking: only one request should succeed', async () => {
    ...
    const { user: ptUser, token: ptToken } = await createUserAndToken('pt');

    // create an availability slot via direct model to keep test deterministic
    const slot = await Appointment.create({ pt: ptUser._id, startTime: new Date(Date.now() + 3600 * 1000), endTime: new Date(Date.now() + 7200 * 1000), status: 'available' });
    ...
    const p1 = request(app).put(`/api/appointments/${slot._id}/book`).set('x-auth-token', t1);
    const p2 = request(app).put(`/api/appointments/${slot._id}/book`).set('x-auth-token', t2);
