...
beforeAll(async () => {
	...
	// Add standard format support (date-time, uri, email, etc.) so schemas with
	// formats validate correctly in CI and locally.
	try {
		const addFormats = require("ajv-formats");
		addFormats(ajv);
	} catch (_err) {
		// If ajv-formats isn't available, tests will still run but may warn
		// about unknown formats. We log a friendly message to help debugging.
		// Do not fail here; the test assertions below are authoritative.
		// eslint-disable-next-line no-console
		console.warn(
			"ajv-formats not available; date-time formats may be ignored.",
		);
	}
});
...
test("Validate POST /api/appointments/availability and GET /api/appointments/me", async () => {
	...
	const pt = await User.create({
		email: "pt+schema@example.com",
		password: "pass",
		role: "pt",
	});
	const ptProfile = await Profile.create({
		user: pt._id,
		firstName: "PT",
		lastName: "Tester",
	});
	pt.profile = ptProfile._id;
	await pt.save();

	const jwt = require("jsonwebtoken");
	const ptToken = jwt.sign(
		{ user: { id: pt._id.toString() } },
		process.env.JWT_SECRET || "testsecret",
	);
	...
	if (schemaPost) {
		...
		if (!ok)
			console.error("Availability POST validation errors:", validate.errors);
		expect(ok).toBe(true);
	}
	...
	if (!okMe)
		console.error("Appointments/me validation errors:", validateMe.errors);
...
test("Booking and review flow: book appointment, confirm, post review (schema checks when available)", async () => {
	...
	const end = new Date(Date.now() + 7200 * 1000).toISOString();
	const payload = { slots: [{ startTime: start, endTime: end }] };
	const _createRes = await request(app)
		.post("/api/appointments/availability")
		.set("x-auth-token", ptToken)
		.send(payload)
		.expect(201);
	...
	if (schemaBook) {
		...
		if (!ok)
			console.error("Booking response validation errors:", validate.errors);
		expect(ok).toBe(true);
	}
	...
	if (schemaStatus) {
		...
		if (!ok)
			console.error("Status response validation errors:", validate.errors);
		expect(ok).toBe(true);
	}

	// Patient posts a review
	const reviewPayload = {
		therapistId: pt._id.toString(),
		rating: 5,
	...
	if (schemaReview) {
		...
		if (!ok)
			console.error("Review response validation errors:", validate.errors);
		expect(ok).toBe(true);
	}
});
